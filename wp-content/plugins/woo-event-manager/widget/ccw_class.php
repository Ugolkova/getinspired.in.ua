...
class ccw_widget extends WP_Widget {
	...
	function __construct() {
		...
		__(CCW_PLUGIN_NAME, 'wooevents'), 
		
		// Widget description
		array( 'description' => __( 'Custom Calendar widget', 'wooevents' ), ) 
		);
	...
	public function widget( $args, $instance ) {
		...
		wp_enqueue_style( 'ccw_tooltipster', CCW_PLUGIN_URL.'css/widget/tooltipster.css' );
		wp_enqueue_style( 'ccw_tooltipster_shadow', CCW_PLUGIN_URL.'css/widget/themes/tooltipster-shadow.css' );
		wp_enqueue_style( 'custom_css', CCW_PLUGIN_URL.'css/widget/custom.css' );
		wp_enqueue_script( 'ccw_js', CCW_PLUGIN_URL.'js/widget/jquery.tooltipster.min.js', array('jquery') );
		...
		$output = $this->cww_get_calendar(true,false,$nextmonths,$previousmonths,$cats);
	 	echo __( $output, 'wooevents' );
		echo '</div>';
		echo '<script language="javascript">jQuery(document).ready(function($) {$(".tooltip_interact").tooltipster({theme: "tooltipster-shadow",animation: "grow",contentAsHTML: true, interactive: true});});</script>';
	...
	public function form( $instance ) {
        ...
        <p>
		<label for="<?php echo $this->get_field_id( 'previousmonths' ); ?>"><?php _e( '# of Previous Months:' ); ?></label> <br />
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['cats'] = ( is_array( $new_instance['cats'] ) ) ? $new_instance['cats'] : array();
		if( in_array( "all", $instance['cats'] ) ){
			unset($instance['cats']);
			$instance['cats'] = array( 0 => 'all' );
		}
		return $instance;
	}
	
	public function cww_get_calendar($initial = true, $echo = true, $nextmonths = CCW_MONTHS_DEFAULT, $previousmonths = CCW_MONTHS_DEFAULT, $cats = array( 0 => 'all' ) ) {
		...
		/*if ( $cache = wp_cache_get( 'get_cww_calendar', 'cww_calendar' ) ) {
			if ( is_array($cache) && isset( $cache[ $key ] ) ) {
				if ( $echo ) {
					
					echo apply_filters( 'get_cww_calendar', $cache[$key] );
					return;
				} else {
					return apply_filters( 'get_cww_calendar', $cache[$key] );
				}
			}
		}
	...
	private function cww_get_month_html($m, $monthnum, $year, $wp_locale, $posts, $show_next=true, $show_prev=true, $cats ) {
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( in_array($day, $daywithpost) ) // any posts today?
				...
				$calendar_output .= count($dayswithposts)."-".$day;
			$calendar_output .= '</td>';
